    ...
    <html lang="en">
      <body>
        <DerivativeCanvasProvider config={derivativeCanvasConfig}>
          <HelloConvoProvider>{children}</HelloConvoProvider>
...
export const HelloConvoProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
...
export default function BoardCanvasPage({ params }: BoardCanvasPageProps) {
  const { summonAgent, importFromShopify, exportToFacebookAds } =
    useHelloConvo();
      ...
      headerComponent={() => (
        <div className="flex items-center justify-between w-full">
          ...
          <div className="flex space-x-2">
            <button
              onClick={() =>
                handleCanvasInteraction("summon-agent", {
                  agentType: "daily-briefing",
                })
              }
            ...
            <button
              onClick={() =>
                handleCanvasInteraction("import-shopify", {
...
const ShopifyImportButton: React.FC = () => {
  ...
  return (
    <>
      ...
      {showDialog && (
        <ShopifyImportDialog onClose={() => setShowDialog(false)} />
      )}
    </>
  );
};

const ShopifyImportDialog: React.FC<{ onClose: () => void }> = ({
  onClose,
}) => {
...
<function_calls> <invoke name="TodoWrite"> <parameter name="todos">[{"content": "Rename framework to 'Derivative Canvas' and update all references", "status": "completed", "activeForm": "Renaming framework to 'Derivative Canvas' and updating all references"}]
